...
from errors import internal_error_response, mt5_error_response, validation_error_response
from flasgger import swag_from
from flask import Blueprint, jsonify, request
from lib import validate_symbol, validate_volume, validate_sl_tp, validate_pending_price
...
def send_market_order_endpoint():
    ...
    try:
        ...
        if not all(field in data for field in required_fields):
            return validation_error_response("Missing required fields", {"required": required_fields})

        if not validate_symbol(data['symbol']):
            return validation_error_response(f"Symbol not found or not selectable: {data['symbol']}")
        ...
        order_type_str = data['type'].upper() if isinstance(data['type'], str) else str(data['type'])
        ...
        if order_type in [mt5.ORDER_TYPE_BUY, mt5.ORDER_TYPE_SELL]:
            ...
            if tick is None:
                return validation_error_response(f"Failed to get symbol price for {data['symbol']}")
            ...
            is_valid, error_msg = validate_pending_price(order_type, data['symbol'], price)
            if not is_valid:
                return validation_error_response(error_msg)
        else:
        ...
        if result is None:
            logger.error(f"order_send returned None for {order_type_str} {data['symbol']}")
            return validation_error_response("Order execution failed - MT5 returned None")

        if result.retcode != mt5.TRADE_RETCODE_DONE:
            return mt5_error_response("Send order", result)

        action_str = "executed" if action == TRADE_ACTION_DEAL else "placed"
        logger.info(f"Order {action_str} successfully: {order_type_str}, symbol={data['symbol']}, volume={volume}, price={price}")
    ...
    'parameters': [
        {
            ...
            'required': True,
            'schema': {
                '$ref': '#/definitions/OrderCheckRequest'
            }
        }
    ],
    'responses': {
        200: {
            ...
            'schema': {
                '$ref': '#/definitions/OrderCheckResponse'
            }
        },
        400: {
...
def order_check_endpoint():
    ...
    try:
        ...
        if result is None:
            return validation_error_response("Order check failed - MT5 returned None"), 400
        ...
        if result.retcode != mt5.TRADE_RETCODE_DONE and result.retcode != 0:
            return jsonify({
                ...
                "margin_free": result.margin_free if hasattr(result, 'margin_free') else None
            }), 400
    ...
    'parameters': [
        {
            ...
            'required': True,
            'schema': {
                '$ref': '#/definitions/MarginCalculationRequest'
            }
        }
    ],
    'responses': {
        200: {
            ...
            'schema': {
                '$ref': '#/definitions/MarginCalculationResponse'
            }
        },
        400: {
...
def order_calc_margin_endpoint():
    ...
    try:
        ...
        if margin is None:
            logger.warning(f"Margin calculation returned None for {data['symbol']}, volume={volume}, price={price}")
    ...
    'parameters': [
        {
            ...
            'required': True,
            'schema': {
                '$ref': '#/definitions/ProfitCalculationRequest'
            }
        }
    ],
    'responses': {
        200: {
            ...
            'schema': {
                '$ref': '#/definitions/ProfitCalculationResponse'
            }
        },
        400: {
...
def order_calc_profit_endpoint():
    ...
    try:
        ...
        order_type_str = data['type'].upper() if isinstance(data['type'], str) else str(data['type'])
        if order_type_str not in ORDER_TYPE_MAP:
            return validation_error_response(f"Invalid order type: {data['type']}. Must be 'BUY' or 'SELL'")
        ...
        profit = mt5.order_calc_profit(mt5_action, data['symbol'], volume, price_open, price_close)

        if profit is None:
            logger.warning(f"Profit calculation returned None for {data['symbol']}, volume={volume}, price_open={price_open}, price_close={price_close}")
            return validation_error_response(
                "Profit calculation unavailable",
                {"reason": "Symbol may not support this calculation"}
            ), 400

        logger.info(f"Profit calculated: symbol={data['symbol']}, volume={volume}, price_open={price_open}, price_close={price_close}, profit={profit}")
...
def get_pending_orders():
    ...
    try:
        ...
        if orders is None:
            logger.error("Failed to retrieve pending orders - mt5.orders_get() returned None")
            return jsonify({
        ...
        for order in orders:
            ...
            order_dict['type_str'] = ORDER_TYPE_TO_STRING.get(order.type, f"UNKNOWN_{order.type}")
            orders_list.append(order_dict)

        logger.info(f"Retrieved {len(orders_list)} pending orders", extra={
...
def cancel_order(ticket):
    ...
    try:
        ...
        if result is None:
            ...
            return validation_error_response("Order cancellation failed - MT5 returned None"), 400

        if result.retcode != mt5.TRADE_RETCODE_DONE:
            logger.error(f"Failed to cancel order {ticket}: retcode={result.retcode}, comment={result.comment}")
...
@swag_from({
    ...
    'parameters': [
        ...
        },
        {
            ...
            'required': True,
            'schema': {
                '$ref': '#/definitions/OrderModificationRequest'
            }
        }
    ],
    'responses': {
...
def modify_order(ticket):
    ...
    try:
        ...
        if 'price' not in data and 'sl' not in data and 'tp' not in data:
            return validation_error_response("At least one of price, sl, or tp must be provided")
        ...
        if new_sl is not None and new_sl < 0:
            return validation_error_response("Stop loss must be non-negative (use 0 to remove)")

        if new_tp is not None and new_tp < 0:
            return validation_error_response("Take profit must be non-negative (use 0 to remove)")

        if 'price' in data:
            is_valid, error_msg = validate_pending_price(order.type, order.symbol, new_price)
            if not is_valid:
                return validation_error_response(error_msg)

        if (new_sl is not None and new_sl > 0) or (new_tp is not None and new_tp > 0):
            is_valid, error_msg = validate_sl_tp(order.type, new_price, new_sl if new_sl > 0 else None, new_tp if new_tp > 0 else None)
            if not is_valid:
                return validation_error_response(error_msg)
        ...
        if result is None:
            ...
            return validation_error_response("Order modification failed - MT5 returned None"), 400

        if result.retcode != mt5.TRADE_RETCODE_DONE:
            logger.error(f"Failed to modify order {ticket}: retcode={result.retcode}, comment={result.comment}")
            return mt5_error_response("Modify order", result)

        logger.info(f"Order {ticket} modified successfully: price={new_price}, sl={new_sl}, tp={new_tp}")
